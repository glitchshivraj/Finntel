      ...
      <div style={{ flex:1, marginLeft:sidebar?228:66, transition:"margin-left .3s cubic-bezier(.16,1,.3,1)", display:"flex", flexDirection:"column", minHeight:"100vh", position:"relative", zIndex:1 }}>
        ...
        <header style={{ height:60, borderBottom:"1px solid rgba(255,255,255,.05)", background:"rgba(5,5,8,.88)", backdropFilter:"blur(20px)", display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 28px", position:"sticky", top:0, zIndex:40 }}>
          ...
          <div style={{ display:"flex", gap:9 }}>
            {highRisk>0 && <div className="chip" style={{ background:"rgba(255,107,91,.1)", border:"1px solid rgba(255,107,91,.25)", color:"#FF6B5B" }}>🚨 {highRisk} HIGH RISK</div>}
            ...
            <button className="btn btn-primary" style={{ padding:"8px 16px" }}>+ New Rule</button>
          </div>
        </header>
            ...
            <div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"repeat(6,1fr)", gap:12, marginBottom:20 }}>
                {[
                  ...
                  <div key={c.label} className={`card fu d${i+1}`} style={{ padding:"18px 20px" }}>
              ...
              <div className="card fu d5" style={{ padding:"20px 22px" }}>
                ...
                <table style={{ width:"100%", borderCollapse:"collapse" }}>
                  <thead><tr>
                    {["Applicant","Amount","Score","Risk","Status","Date"].map(h=><th key={h} className="th" style={{ textAlign:"left" }}>{h}</th>)}
                  </tr></thead>
                  <tbody>
              ...
              <div className="card fu d2">
                ...
                <table style={{ width:"100%", borderCollapse:"collapse" }}>
                  <thead><tr style={{ borderBottom:"1px solid rgba(255,255,255,.05)" }}>
                    {["Applicant","Amount","Score","Risk","Status","Date","Actions"].map(h=><th key={h} className="th" style={{ textAlign:"left" }}>{h}</th>)}
                  </tr></thead>
                  <tbody>
                    {filtered.map(a=>(
        ...
        <div className="modal-bg" onClick={e=>{if(e.target===e.currentTarget){setOA(null);setOR("")}}}>
          <div className="modal">
            ...
            <div style={{ display:"flex", gap:10 }}>
              ...
              <button className="btn btn-ghost" style={{ padding:"12px 20px" }} onClick={()=>{setOA(null);setOR("")}}>Cancel</button>
            </div>
          </div>
        </div>
      )}
