    ...
    <div style={{ fontFamily: "'Outfit', sans-serif", background: "#050508", color: "#F0EEFF", minHeight: "100vh", display: "flex", flexDirection: "column", cursor: "none", overflowX: "hidden" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .gradient-text {
          background: linear-gradient(135deg, #FF6BFF 0%, #A855F7 30%, #00D4FF 65%, #00FFB3 100%);
        ...
        .auth-input:focus {
          border-color: rgba(255,107,255,0.45);
          background: rgba(255,107,255,0.04);
          box-shadow: 0 0 0 3px rgba(255,107,255,0.08), 0 0 20px rgba(255,107,255,0.1);
        }
        ...
        .google-btn:hover {
          ...
          box-shadow: 0 12px 36px rgba(255,107,255,0.14);
        }
      ...
      <div style={{ position:"fixed", width:320, height:320, borderRadius:"50%", background:"radial-gradient(circle, rgba(168,85,247,0.07) 0%, transparent 70%)", top:"50%", left:"30%", animation:"orbFloat1 16s ease-in-out infinite reverse", pointerEvents:"none", zIndex:0 }} />
            ...
            <div style={{ position:"absolute", inset:0, borderRadius:9, background:"linear-gradient(135deg, #FF6BFF, #00D4FF)", padding:1.5 }}>
              <div style={{ width:"100%", height:"100%", background:"#080810", borderRadius:8, display:"flex", alignItems:"center", justifyContent:"center" }}>
                <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
                  <path d="M9 1.5L15.5 5V9C15.5 13 12.5 16.5 9 17.5C5.5 16.5 2.5 13 2.5 9V5L9 1.5Z" fill="url(#nav-g)" />
                  <defs><linearGradient id="nav-g" x1="2.5" y1="1.5" x2="15.5" y2="17.5" gradientUnits="userSpaceOnUse"><stop stopColor="#FF6BFF"/><stop offset="1" stopColor="#00D4FF"/></linearGradient></defs>
                </svg>
              </div>
            </div>
          </div>
        ...
        <div style={{ width:"100%", maxWidth:440 }}>
          ...
          <div className="fade-up d1" style={{ textAlign:"center", marginBottom:32 }}>
            <h1 style={{ fontSize:"clamp(30px,4vw,42px)", fontWeight:900, letterSpacing:"-0.04em", lineHeight:1.05, marginBottom:10 }}>
              {tab === "login"
                ? <>Welcome back to <span className="gradient-text">Finntel</span></>
                : <>Join <span className="gradient-text">Finntel</span></>}
            </h1>
            <p style={{ color:"rgba(240,238,255,0.42)", fontSize:15, lineHeight:1.7 }}>
              {tab === "login"
                ? "Sign in to access your risk assessment dashboard."
                : "Start evaluating loan risk with AI-powered decisions."}
            </p>
          </div>
          ...
          <div
            ...
            style={{ background:"rgba(12,10,20,0.85)", border:"1px solid rgba(255,107,255,0.16)", borderRadius:24, padding:34, backdropFilter:"blur(28px)", position:"relative", overflow:"hidden", boxShadow:"0 40px 80px rgba(255,107,255,0.07), 0 0 0 1px rgba(255,107,255,0.07)" }}
          >
            {/* Scan line */}
            ...
            <div style={{ position:"absolute", top:-60, left:"50%", transform:"translateX(-50%)", width:260, height:120, borderRadius:"50%", background:"rgba(255,107,255,0.08)", filter:"blur(40px)", pointerEvents:"none" }} />
            ...
            <div style={{ display:"flex", gap:5, background:"rgba(255,255,255,0.035)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:13, padding:5, marginBottom:28, position:"relative", zIndex:1 }}>
              {(["login","signup"] as const).map((t) => (
                <button key={t} className="tab-btn" onClick={() => setTab(t)}
                  style={{
                    background: tab === t ? "linear-gradient(135deg, rgba(255,107,255,0.22), rgba(168,85,247,0.18), rgba(0,212,255,0.14))" : "transparent",
                    color: tab === t ? "#F0EEFF" : "rgba(240,238,255,0.36)",
                    border: tab === t ? "1px solid rgba(255,107,255,0.28)" : "1px solid transparent",
                    boxShadow: tab === t ? "0 0 20px rgba(255,107,255,0.14)" : "none",
                  }}>
            ...
            <div key={tab} className="form-slide" style={{ position:"relative", zIndex:1 }}>
              {tab === "login" ? (
                <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
                  <div>
                    ...
                    <input className="auth-input" type="email" placeholder="Enter your email address" value={loginEmail} onChange={e => setLoginEmail(e.target.value)} />
                  </div>
                  <div>
                    ...
                    <div style={{ position:"relative" }}>
                      <input className="auth-input" type={showPass ? "text" : "password"} placeholder="Enter your account password" style={{ paddingRight:48 }} value={loginPass} onChange={e => setLoginPass(e.target.value)} />
                      <button onClick={() => setShowPass(!showPass)} style={{ position:"absolute", right:14, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", color:"rgba(240,238,255,0.32)", cursor:"none", padding:4, transition:"color 0.2s" }}
                        ...
                        onMouseLeave={e => (e.currentTarget.style.color = "rgba(240,238,255,0.32)")}>
                        {showPass
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                    <div style={{ width:18, height:18, borderRadius:5, border:"1px solid rgba(255,107,255,0.25)", background:"rgba(255,107,255,0.05)", flexShrink:0, cursor:"none" }} />
                    <span style={{ fontSize:13, color:"rgba(240,238,255,0.44)", fontWeight:600 }}>Remember me for 30 days</span>
                  </div>
                </div>
              ) : (
                <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
                  <div>
                    ...
                    <input className="auth-input" type="text" placeholder="Enter your full name" value={signupName} onChange={e => setSignupName(e.target.value)} />
                  </div>
                  <div>
                    ...
                    <input className="auth-input" type="email" placeholder="Enter your work email" value={signupEmail} onChange={e => setSignupEmail(e.target.value)} />
                  </div>
                  <div>
                    ...
                    <div style={{ position:"relative" }}>
                      <input className="auth-input" type={showPass ? "text" : "password"} placeholder="Create a strong password" style={{ paddingRight:48 }} value={signupPass} onChange={e => setSignupPass(e.target.value)} />
                      <button onClick={() => setShowPass(!showPass)} style={{ position:"absolute", right:14, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", color:"rgba(240,238,255,0.32)", cursor:"none", padding:4, transition:"color 0.2s" }}
                        onMouseEnter={e => (e.currentTarget.style.color = "#FF6BFF")}
                        onMouseLeave={e => (e.currentTarget.style.color = "rgba(240,238,255,0.32)")}>
                        {showPass
                    ...
                    {signupPass && (
                      <div style={{ marginTop:10 }}>
                        <div style={{ display:"flex", gap:5, marginBottom:6 }}>
                          {[1,2,3,4].map(n => (
                            <div key={n} style={{ flex:1, height:3, borderRadius:2, background: n <= strength ? strengthColor[strength] : "rgba(255,255,255,0.07)", transition:"background 0.3s", boxShadow: n <= strength ? `0 0 8px ${strengthColor[strength]}55` : "none" }} />
                          ))}
                        </div>
                  ...
                  <div>
                    ...
                    <div style={{ position:"relative" }}>
                      <input
                        ...
                        style={{ paddingRight:48, borderColor: signupConfirm && signupPass !== signupConfirm ? "rgba(255,107,107,0.4)" : undefined }}
                        value={signupConfirm}
                        onChange={e => setSignupConfirm(e.target.value)}
                      />
                      {signupConfirm && signupPass === signupConfirm ? (
                        <div className="check-animate" style={{ position:"absolute", right:14, top:"50%", transform:"translateY(-50%)" }}>
                          <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#00FFB3" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>
                        </div>
                      ) : (
                        <button onClick={() => setShowConfirm(!showConfirm)} style={{ position:"absolute", right:14, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", color:"rgba(240,238,255,0.32)", cursor:"none", padding:4, transition:"color 0.2s" }}
                          onMouseEnter={e => (e.currentTarget.style.color = "#FF6BFF")}
                          onMouseLeave={e => (e.currentTarget.style.color = "rgba(240,238,255,0.32)")}>
                          <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                        </button>
                    ...
                    {signupConfirm && signupPass !== signupConfirm && (
                      <p style={{ fontSize:12, color:"#FF6B5B", marginTop:6, fontWeight:600 }}>Passwords don't match</p>
                    )}
                  </div>
                  <div style={{ display:"flex", alignItems:"flex-start", gap:10 }}>
                    <div
                      onClick={() => setAgree(!agree)}
                      style={{ width:18, height:18, borderRadius:5, border:`1px solid ${agree ? "#FF6BFF" : "rgba(255,107,255,0.22)"}`, background: agree ? "rgba(255,107,255,0.14)" : "rgba(255,107,255,0.04)", display:"flex", alignItems:"center", justifyContent:"center", cursor:"none", flexShrink:0, marginTop:2, transition:"all 0.25s", boxShadow: agree ? "0 0 12px rgba(255,107,255,0.28)" : "none" }}
                    >
                    ...
                    <span style={{ fontSize:13, color:"rgba(240,238,255,0.42)", lineHeight:1.65, fontWeight:500 }}>
                      I agree to the{" "}
                      <a href="#" style={{ color:"#FF6BFF", textDecoration:"none", fontWeight:700, cursor:"none" }}>Terms of Service</a>{" "}
                      and{" "}
                      <a href="#" style={{ color:"#FF6BFF", textDecoration:"none", fontWeight:700, cursor:"none" }}>Privacy Policy</a>
                    </span>
                  </div>
                </div>
              )}
              ...
              {/* Submit */}
              <button className="cta-btn" style={{ marginTop:22 }} onClick={handleSubmit} disabled={loading}>
                {loading ? (
              ...
              <p style={{ textAlign:"center", marginTop:18, fontSize:14, color:"rgba(240,238,255,0.38)" }}>
                ...
                <button
                  ...
                  style={{ background:"none", border:"none", color:"#FF6BFF", fontFamily:"'Outfit',sans-serif", fontWeight:800, fontSize:14, cursor:"none", padding:0, transition:"all 0.2s" }}
                  onMouseEnter={e => (e.currentTarget.style.textShadow = "0 0 16px rgba(255,107,255,0.6)")}
      ...
      <div style={{ position:"relative", zIndex:1, borderTop:"1px solid rgba(255,255,255,0.04)", padding:"16px 48px", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:12 }}>
        <span className="mono" style={{ fontSize:11, color:"rgba(240,238,255,0.18)" }}>© 2024 Finntel Technologies</span>
        <div style={{ display:"flex", gap:24 }}>
          {["Privacy","Terms","Support"].map(l => (
            <a key={l} href="#" style={{ color:"rgba(240,238,255,0.22)", fontSize:12, textDecoration:"none", fontWeight:700, cursor:"none", transition:"color 0.2s" }}
              onMouseEnter={e => (e.currentTarget.style.color = "#FF6BFF")}
              onMouseLeave={e => (e.currentTarget.style.color = "rgba(240,238,255,0.22)")}>{l}</a>
          ))}
        </div>
      </div>
    </div>
